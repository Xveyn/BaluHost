...
- [ ] **Workflow secrets**: New `secrets.*` references? Confirm the secret exists, is scoped correctly, and is not echoed/logged. Read a secret via `env:`, never by interpolating `${{ secrets.X }}` into a `run:` body.
- [ ] **Third-party action pin**: Does a new `uses:` reference an action outside `actions/*`? It must be pinned to a full commit SHA with a trailing `# vX.Y.Z` comment (#333), so `.github/dependabot.yml` can bump it. A bare tag on a third-party action is a moving trust root — block, especially in a job holding `contents: write` or a deploy key.
...
8. **`ci-runner` and its containers have unrestricted egress** — A maliciously approved PR can exfiltrate the workdir contents and make outbound calls to arbitrary hosts. Mitigations: the `ci-tests` environment gate (manual approval), the limited blast radius (workdir contains only PR code), no production secrets reachable. Future tightening: egress firewall allowing only `pypi.org`, `files.pythonhosted.org`, `registry.npmjs.org`, `api.github.com`, `objects.githubusercontent.com`, `registry.docker.io`, `mcr.microsoft.com` and `*.data.mcr.microsoft.com`. The last two are the Playwright image and its blobs, needed since `mock-e2e` joined the sandbox (#330/#423) — a firewall built from the pre-E2E list would silently break that job. `docs/CI.md` ("Egress expectations") carries the authoritative list with the reason per host; keep the two in sync, or drop this enumeration and point there.
...
11. **`baluhost` user may run `systemctl reboot` as root** — Added in `deploy/install/templates/sudoers-baluhost-power` (synced to an installed box only via `deploy/scripts/install-power-sudoers.sh`, itself only invoked from a `SYNC_PERMISSIONS=1` deploy) for the scheduled maintenance reboot (`backend/app/services/power/scheduled_reboot.py`). Exactly one verb, no arguments, no wildcards — but this is the widest entry in this file: unlike every other grant here, it terminates every service on the box, not one systemd unit. The compensating controls: the feature is **off by default** — a missing or disabled `scheduler_configs` row for `system_reboot` is treated as "off" in its own explicit branch on both paths that ask the question, `SchedulerService._check_scheduler_enabled()` (status/reporting) and `reboot_state.load_enabled_config()` (the tick that actually arms and fires); only an admin can read, configure, toggle, or preview it (`get_current_admin` on every `/api/schedulers/system_reboot/*` route); and four gates — active core uptime, a display powered on, another scheduler job running, a non-idle system — must all be open before `scheduled_reboot.gates_blocking()` lets it proceed. Every configuration change (`update_reboot_schedule`), every toggle (`toggle_reboot_schedule`), and every actual reboot (`scheduled_reboot`) is written to the audit log. Until the one-time `SYNC_PERMISSIONS=1` deploy lands the sudoers line, the reboot fails closed — `run_reboot_command()` gets `sudo: no entry` back, reports it as `SKIP_REBOOT_FAILED`, and the admin sees it via the `reboot.skipped` notification instead of the box silently never rebooting.
