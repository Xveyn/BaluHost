...
| `lifespan.py` | FastAPI lifespan: startup/shutdown orchestration. Primary-worker election via file lock (`PRIMARY_LOCK_PATH`; the local channel never becomes primary). `_start_ws_bus`/`_stop_ws_bus` run in **every** worker, not just the primary — each holds its own sockets (#685). Starts hardware services, plugins, mDNS, heartbeat writer. `IS_PRIMARY_WORKER` flag controls which process runs hardware tasks. Also runs `_reconcile_plugin_enablement()` on the primary every 15s (#465) — plugin background tasks can only start there, and relying on the request-path reconcile made that depend on frontend polling, which stops when the browser tab is hidden. Spawned deliberately **after** `load_enabled_plugins()`: its first tick is immediate and would otherwise race that call |
...
| `concurrency_probe.py` | Instrumentation for S1/#300: event-loop lag, in-flight requests, DB-pool and anyio-threadpool utilization. One lifespan task per worker reports a window on the `baluhost.concurrency` logger every 60s. Pool **checkouts** are not sampled but accounted by two engine-level SQLAlchemy listeners (`PoolCheckoutTracker`) — a point sampler on the loop is structurally blind while a non-awaiting handler blocks it. Those listeners sit in the checkout path of every DB operation and must never raise; keep them exception-proof. Both halves (task + `InFlightMiddleware`) are gated on `CONCURRENCY_PROBE_ENABLED`. See `docs/monitoring/CONCURRENCY_PROBE.en.md` |
