...
| `concurrency_probe.py` | Instrumentation for S1/#300: event-loop lag, in-flight requests, DB-pool and anyio-threadpool utilization. One lifespan task per worker reports a window on the `baluhost.concurrency` logger every 60s. See `docs/monitoring/CONCURRENCY_PROBE.en.md` |
