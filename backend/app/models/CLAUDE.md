...
**Power & Hardware**: `power.py` (profiles, demands, auto-scaling), `power_preset.py`, `fans.py` (config, samples, schedules, curve profiles), `sleep.py`, `smart_device.py`, `power_permissions.py` (UI name: "System Permissions / Systemberechtigungen"; table/columns keep the `power_permissions` name; incl. `can_launch_games`), `gpu_power.py`, `power_boost_rule.py`
