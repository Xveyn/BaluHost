...
- `fan_gpu_acoustics_store.py` — Singleton-row/JSON persistence for the GPU acoustics config (`desired` + observed `baseline`), same pattern as the GPU power config store
