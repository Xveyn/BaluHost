 ...
 *
 * NOTE: This use case still has Android dependencies (Context, Base64)
...
class ImportVpnConfigUseCase @Inject constructor(
    ...
    suspend operator fun invoke(configBase64: String, autoRegister: Boolean = true): Result<VpnConfig> {
        return try {
            ...
            logger.debug(TAG, "Importing VPN config (${configString.length} bytes)")
            ...
            logger.debug(TAG, "VPN config parsed: IP=${config.assignedIp}, Endpoint=${config.serverEndpoint}")

            // Auto-register VPN tunnel if requested
            if (autoRegister) {
                try {
                    prepareVpnTunnel(config)
                    logger.debug(TAG, "VPN tunnel prepared successfully")
                } catch (e: Exception) {
                    logger.warn(TAG, "VPN tunnel preparation failed (will require manual setup): ${e.message}")
            ...
            logger.error(TAG, "Failed to import VPN config", e)
    ...
    private fun prepareVpnTunnel(config: VpnConfig) {
        ...
        logger.debug(TAG, "Storing VPN tunnel metadata for ${config.serverEndpoint}")
