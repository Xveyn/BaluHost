...
| `usePiDashboardData.ts` | `api/pi` | BaluPi dashboard (Pi build only): handshake / energy-current / system-status / snapshot via **four independent `useQuery`** (default 30s poll each, `pi.*` keys). Independent queries reproduce the page's old `Promise.allSettled` semantics — one endpoint failing leaves the others intact. Returns `{ handshake, energy, piSystem, snapshot, refreshing, refetch }` (`refreshing` = any `isFetching`; `refetch` re-requests all four). Consumer: `PiDashboard` (WoL stays imperative via `sendNasWol`) |
