...
| `usePowerManagementData.ts` | `api/power-management` | Combined data for the `PowerManagement` page via **TanStack Query** (`useQuery`, default 5s poll, `power.management()` key). queryFn fans out to the seven power endpoints via `Promise.all` → all-or-nothing (any failure = error, last good snapshot retained). Returns `{ status, presets, demands, intensities, history, autoScaling, dynamicConfig, loading, error, lastUpdated, refetch }`; `refetch` resolves to a success boolean (like `useRaidStatus`) so the page toasts only on success. Page mutations stay imperative and call `refetch()` |
