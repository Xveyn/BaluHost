...
| `useSmartMode.ts` | `api/system`, `api/smart` | Dev-only SMART data source toggle (mock↔real) for the dashboard via **TanStack Query**: system-mode read (`system.mode()`, `staleTime: Infinity`) gates a `smart.mode()` read (`enabled: isDevMode`, `staleTime: 60s`); `toggle` is a **`useMutation`** that republishes the new mode via `setQueryData(smart.mode())` and invalidates `smart.status()` to refresh the SMART disk data. Shares both keys with the scheduler `MaintenancePanel` → one cache entry, no duplicate fetch (#299). Returns `{ smartMode, isDevMode, toggle, isToggling }`; consumer: `Dashboard` |
