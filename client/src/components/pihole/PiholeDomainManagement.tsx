...
export default function PiholeDomainManagement() {
  ...
  const TABS: { key: ListType; labelKey: string }[] = [
    { key: 'deny_exact', labelKey: 'domains.tabs.denyExact' },
    { key: 'deny_regex', labelKey: 'domains.tabs.denyRegex' },
    { key: 'allow_exact', labelKey: 'domains.tabs.allowExact' },
    { key: 'allow_regex', labelKey: 'domains.tabs.allowRegex' },
  ];
  ...
  const fetchDomains = useCallback(async () => {
    ...
    try {
      ...
      toast.error(err?.response?.data?.detail || t('domains.loadFailed'));
    } finally {
      setLoading(false);
    }
  }, [activeTab, t]);
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(t('domains.addedToast'));
      setNewDomain('');
      await fetchDomains();
    } catch (err: any) {
      toast.error(err?.response?.data?.detail || t('domains.addFailed'));
    } finally {
      setAdding(false);
    }
  };
  ...
  const handleRemove = async (id: number, domain: string) => {
    ...
    try {
      ...
      toast.success(t('domains.removedToast', { domain }));
      await fetchDomains();
    } catch (err: any) {
      toast.error(err?.response?.data?.detail || t('domains.removeFailed'));
    } finally {
      setRemovingId(null);
    }
  };
      ...
      <div className="p-4">
        ...
        <div className="mb-4 max-h-80 overflow-y-auto overflow-x-auto">
          {loading ? (
            ...
            <p className="py-8 text-center text-sm text-slate-500">{t('domains.noDomains')}</p>
          ) : (
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b border-slate-700/50 text-xs uppercase text-slate-500">
                  <th className="pb-2 pr-4">{t('domains.columns.domain')}</th>
                  <th className="pb-2 pr-4">{t('domains.columns.comment')}</th>
                  <th className="pb-2 w-10" />
                </tr>
              </thead>
              <tbody>
                {domains.map((d) => (
                  <tr key={d.id} className="border-b border-slate-700/30 hover:bg-slate-700/20">
                    <td className="py-2 pr-4 font-mono text-xs text-slate-200">{d.domain}</td>
                    <td className="py-2 pr-4 text-xs text-slate-500">{d.comment || '—'}</td>
                    <td className="py-2">
                      <button
        ...
        <form onSubmit={handleAdd} className="flex items-center gap-2">
          <input
            ...
            placeholder={
              activeTab.includes('regex') ? t('domains.regexPlaceholder') : t('domains.domainPlaceholder')
            }
