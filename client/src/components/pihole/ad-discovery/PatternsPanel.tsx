...
export default function PatternsPanel() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      toast.error(t('adDiscovery.patterns.loadFailed'));
    } finally {
      setLoading(false);
    }
  }, [t]);
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      toast.success(t('adDiscovery.patterns.patternCreated'));
      setShowAdd(false);
      setNewPattern({ pattern: '', is_regex: false, weight: 0.5, category: 'custom' });
      fetchData();
    } catch (err: unknown) {
      const e = err as { response?: { data?: { detail?: string } } };
      toast.error(e?.response?.data?.detail || t('adDiscovery.patterns.createFailed'));
    }
  };
  ...
  const handleToggle = async (p: PatternEntry) => {
    try {
      ...
      toast.error(t('adDiscovery.patterns.toggleFailed'));
    }
  };
  ...
  const handleDelete = async (p: PatternEntry) => {
    ...
    try {
      ...
      toast.success(t('adDiscovery.patterns.patternDeleted'));
      fetchData();
    } catch (err: unknown) {
      const e = err as { response?: { data?: { detail?: string } } };
      toast.error(e?.response?.data?.detail || t('adDiscovery.patterns.deleteFailed'));
    }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-medium text-slate-300">{t('adDiscovery.patterns.title')}</h3>
        <button
          onClick={() => setShowAdd(!showAdd)}
          className="flex items-center gap-1.5 rounded-md bg-slate-700 px-3 py-1.5 text-sm text-white hover:bg-slate-600"
        >
          <Plus className="h-4 w-4" /> {t('adDiscovery.patterns.addPattern')}
        </button>
      </div>
        ...
        <div className="rounded-lg border border-slate-700 bg-slate-800/60 p-4 space-y-3">
          <div className="flex gap-3">
            <input
              ...
              placeholder={t('adDiscovery.patterns.patternPlaceholder')}
            ...
            <label className="flex items-center gap-2 text-sm text-slate-300">
              <input
                ...
                onChange={(e) => setNewPattern((p) => ({ ...p, is_regex: e.target.checked }))}
              />{' '}
              {t('adDiscovery.patterns.regex')}
            </label>
          </div>
          <div className="flex gap-3">
            <div className="flex items-center gap-2">
              <span className="text-xs text-slate-400">{t('adDiscovery.patterns.weight')}</span>
              <input
                type="range"
                min="0.1"
                max="1.0"
                ...
                onChange={(e) => setNewPattern((p) => ({ ...p, weight: parseFloat(e.target.value) }))}
                className="w-24"
              />
      ...
      <div className="overflow-x-auto rounded-lg border border-slate-700/50">
        <table className="w-full text-left text-sm">
          <thead className="border-b border-slate-700 bg-slate-800/60">
            <tr>
              <th className="px-3 py-2 text-slate-400 font-medium">{t('adDiscovery.patterns.columns.pattern')}</th>
              <th className="px-3 py-2 text-slate-400 font-medium">{t('adDiscovery.patterns.columns.type')}</th>
              <th className="px-3 py-2 text-slate-400 font-medium">{t('adDiscovery.patterns.columns.weight')}</th>
              <th className="px-3 py-2 text-slate-400 font-medium">{t('adDiscovery.patterns.columns.category')}</th>
              <th className="px-3 py-2 text-slate-400 font-medium">{t('adDiscovery.patterns.columns.enabled')}</th>
              <th className="px-3 py-2 text-slate-400 font-medium">{t('adDiscovery.patterns.columns.actions')}</th>
            </tr>
          </thead>
            ...
            {loading ? (
              <tr>
                <td colSpan={6} className="px-3 py-6 text-center text-slate-500">
                  {t('adDiscovery.patterns.loading')}
                </td>
              </tr>
            ) : (
              patterns.map((p) => (
                  ...
                  <td className="px-3 py-2">
                    {p.is_regex ? (
                      <span className="flex items-center gap-1 text-xs text-purple-300">
                        <Code className="h-3 w-3" /> {t('adDiscovery.patterns.regexLabel')}
                      </span>
                    ) : (
                      <span className="flex items-center gap-1 text-xs text-slate-400">
                        <Type className="h-3 w-3" /> {t('adDiscovery.patterns.substring')}
                      </span>
                    )}
                  </td>
                  <td className="px-3 py-2 text-slate-300">{p.weight.toFixed(1)}</td>
                  <td className="px-3 py-2">
                    <span className="rounded-full bg-slate-700 px-2 py-0.5 text-xs text-slate-300">{p.category}</span>
                  </td>
                  <td className="px-3 py-2">
                    <button
