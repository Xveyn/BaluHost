...
import {
  ...
  undeployCustomList,
  exportCustomList,
  getCustomListDomains,
  addCustomListDomains,
  removeCustomListDomain,
  type CustomListEntry,
  type CustomListDomainEntry,
...
export default function CustomListsPanel() {
  ...
  const fetchLists = useCallback(async () => {
    ...
    try {
      ...
      toast.error(t('adDiscovery.customLists.loadFailed'));
    } finally {
      setLoading(false);
    }
  }, [t]);
  ...
  const fetchDomains = useCallback(
    async (listId: number, page = 1) => {
      try {
        const result = await getCustomListDomains(listId, page);
        setDomains(result.domains);
        setDomainsPage(page);
      } catch {
        toast.error(t('adDiscovery.customLists.loadDomainsFailed'));
      }
    },
    [t]
  );
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      toast.success(t('adDiscovery.customLists.createdToast'));
      setShowCreate(false);
      setNewList({ name: '', description: '' });
      fetchLists();
    } catch (err: unknown) {
      const e = err as { response?: { data?: { detail?: string } } };
      toast.error(e?.response?.data?.detail || t('adDiscovery.customLists.createFailed'));
    }
  };
  ...
  const handleDelete = async (id: number) => {
    try {
      ...
      toast.success(t('adDiscovery.customLists.deletedToast'));
      if (expandedId === id) setExpandedId(null);
      fetchLists();
    } catch (err: unknown) {
      const e = err as { response?: { data?: { detail?: string } } };
      toast.error(e?.response?.data?.detail || t('adDiscovery.customLists.deleteFailed'));
    }
  };
  ...
  const handleDeploy = async (id: number) => {
    try {
      ...
      toast.success(t('adDiscovery.customLists.deployedToast'));
      fetchLists();
    } catch (err: unknown) {
      const e = err as { response?: { data?: { detail?: string } } };
      toast.error(e?.response?.data?.detail || t('adDiscovery.customLists.deployFailed'));
    }
  };
  ...
  const handleUndeploy = async (id: number) => {
    try {
      ...
      toast.success(t('adDiscovery.customLists.undeployedToast'));
      fetchLists();
    } catch (err: unknown) {
      const e = err as { response?: { data?: { detail?: string } } };
      toast.error(e?.response?.data?.detail || t('adDiscovery.customLists.undeployFailed'));
    }
  };
  ...
  const handleExport = async (id: number, name: string) => {
    try {
      ...
      toast.error(t('adDiscovery.customLists.exportFailed'));
    }
  };
  ...
  const handleAddDomains = async (listId: number) => {
    ...
    try {
      ...
      toast.success(t('adDiscovery.customLists.addedDomainsToast', { count: domainsArr.length }));
      setNewDomains('');
      fetchDomains(listId, domainsPage);
      fetchLists();
    } catch (err: unknown) {
      const e = err as { response?: { data?: { detail?: string } } };
      toast.error(e?.response?.data?.detail || t('adDiscovery.customLists.addDomainsFailed'));
    }
  };
  ...
  const handleRemoveDomain = async (listId: number, domain: string) => {
    try {
      ...
      toast.success(t('adDiscovery.customLists.removedDomainToast'));
      fetchDomains(listId, domainsPage);
      fetchLists();
    } catch {
      toast.error(t('adDiscovery.customLists.removeDomainFailed'));
    }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-medium text-slate-300">{t('adDiscovery.customLists.title')}</h3>
        <button
          onClick={() => setShowCreate(!showCreate)}
          className="flex items-center gap-1.5 rounded-md bg-slate-700 px-3 py-1.5 text-sm text-white hover:bg-slate-600"
        >
          <Plus className="h-4 w-4" /> {t('adDiscovery.customLists.createList')}
        </button>
      </div>
      ...
      {showCreate && (
        <div className="rounded-lg border border-slate-700 bg-slate-800/60 p-4 space-y-3">
          <input
            ...
            placeholder={t('adDiscovery.customLists.namePlaceholder')}
          ...
          <input
            ...
            placeholder={t('adDiscovery.customLists.descriptionPlaceholder')}
            className="w-full rounded-md border border-slate-600 bg-slate-700 px-3 py-1.5 text-sm text-white"
          />
          <button onClick={handleCreate} className="rounded-md bg-blue-600 px-4 py-1.5 text-sm text-white hover:bg-blue-500">
            {t('adDiscovery.customLists.create')}
          </button>
        </div>
      )}

      {loading ? (
        <p className="text-center text-sm text-slate-500">{t('adDiscovery.customLists.loading')}</p>
      ) : lists.length === 0 ? (
        <p className="text-center text-sm text-slate-500">{t('adDiscovery.customLists.empty')}</p>
      ) : (
            ...
            <div key={l.id} className="rounded-lg border border-slate-700 bg-slate-800/40">
              <div className="flex items-center justify-between p-4">
                ...
                <div className="flex items-center gap-1">
                  {l.deployed ? (
                    <button
                      onClick={() => handleUndeploy(l.id)}
                      title={t('adDiscovery.customLists.actions.undeploy')}
                      className="rounded p-1.5 text-orange-400 hover:bg-orange-500/10"
                    >
                      <Rocket className="h-4 w-4" />
                    </button>
                  ) : (
                    <button
                      onClick={() => handleDeploy(l.id)}
                      title={t('adDiscovery.customLists.actions.deploy')}
                  ...
                  <button
                    ...
                    title={t('adDiscovery.customLists.actions.export')}
                  ...
                  <button
                    ...
                    title={t('adDiscovery.customLists.actions.delete')}
                  ...
                  <div className="max-h-64 overflow-y-auto space-y-1">
                    {domains.map((d: CustomListDomainEntry) => (
                      <div key={d.id} className="flex items-center justify-between rounded bg-slate-800/60 px-3 py-1.5">
                        ...
                        <button onClick={() => handleRemoveDomain(l.id, d.domain)} className="text-slate-500 hover:text-red-400">
                          <X className="h-3.5 w-3.5" />
                        </button>
                      </div>
                    ))}
                    {domains.length === 0 && (
                      <p className="text-center text-xs text-slate-500 py-2">{t('adDiscovery.customLists.noDomains')}</p>
                    )}
                  </div>
                </div>
              )}
