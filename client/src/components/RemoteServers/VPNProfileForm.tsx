...
export function VPNProfileForm({ onCreateProfile, isLoading = false }: VPNProfileFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      handleApiError(err, 'Failed to create VPN profile');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      ...
      <Modal isOpen={open} onClose={() => setOpen(false)} title={t('vpn.addProfile')} size="md">
        <p className="text-sm text-slate-400 mb-4">{t('vpn.addProfileDescription')}</p>
