...
- ~~**Medium**: `backend/app/services/backup.py` — Add config files to backup and implement restore flow for full-system restores.~~ — ✅ Resolved (#176). `includes_config` now writes a secret-free settings snapshot (`config/settings.snapshot.json`, secret-named fields redacted) instead of an empty dir with a false-success audit. DB-stored config is already covered by the database backup; restore extracts the snapshot to a `restored-config-*` reference dir and never auto-applies it (overwriting live env/secrets is unsafe).
