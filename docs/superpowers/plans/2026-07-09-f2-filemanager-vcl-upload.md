...
- Quota + user-root-usage query errors are **silently swallowed** (matches the original `catch {}`). The quota warning/critical **toast** fires from an effect on the quota data (preserves the mount + post-upload toast).
