...
Without that sync the writes still succeed. `_write_hwmon_file` (`backend/app/services/power/fan_backend_linux.py`) falls back to `sudo -n tee` on `EACCES`, and `deploy/install/templates/baluhost-hardware-sudoers` grants `tee` on the four `hwmon[0-9]*/device/gpu_od/fan_ctrl/` nodes by name. Until #570 it carried a single `/usr/bin/tee /sys/class/hwmon/*` instead: a wildcard in a sudoers *argument* matches `/` (only the command path itself is exempt), so that entry covered the write path — and every other path reachable from there, including `../` escapes out of `/sys/class/hwmon/`. It was narrowed for that reason; the acoustics writes still work because the four nodes are now listed explicitly. What the udev rule buys is an unprivileged write path, not the feature working at all.
...
- ~~**The sudoers entry `/usr/bin/tee /sys/class/hwmon/*` is broader than needed.**~~ Resolved in #570: measured on the reference box, the entry also permitted `../` escapes (a root write to any file) and device control such as `device/driver/unbind` — including on the NVMe controllers. Both `tee` rules now enumerate their targets with character classes and no `*`; `backend/tests/test_hardware_sudoers_scope.py` holds the template against both the permitted and the forbidden paths.
